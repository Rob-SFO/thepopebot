...
export async function runClusterRole(roleData, payload, trigger) {
  ...
  // Pass ANTHROPIC_* vars for custom endpoints (UltimateAI, etc.)
  const anthropicBaseUrl = getConfig('ANTHROPIC_BASE_URL');
  if (anthropicBaseUrl) {
    env.push(`ANTHROPIC_BASE_URL=${anthropicBaseUrl}`);
  }
